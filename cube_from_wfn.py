...
parser.add_argument(
    ...
    default=14.0,
    help=(
        "Size of the region around the atom where each"
...
parser.add_argument(
    ...
    action="store_true",
    help=(
        "Calculate charge density with 'fake' artificial core (all occ orbitals are evaluated)."
    ),
)
parser.add_argument(
    "--spin_dens",
...
### -----------------------------------------------------------
parser.add_argument(
    "--do_not_center_atoms", action="store_true", help=("Center atoms to cell.")
)
### -----------------------------------------------------------
...
for imo in np.arange(n_homo + n_lumo):
    ...
    for ispin in range(mol_grid_orb.nspin):
        ...
        if args.orb_square:
            mol_grid_orb.write_cube(
                output_dir + name + "_sq.cube", i_rel_homo, spin=ispin, square=True
            )

if args.charge_dens:
    mol_grid_orb.calculate_and_save_charge_density(output_dir + "charge_density.cube")
if args.charge_dens_artif_core:
    mol_grid_orb.calculate_and_save_charge_density(
        output_dir + "charge_density_artif.cube", artif_core=True
    )
...
print(
    "R%d/%d: finished, total time: %.2fs" % (mpi_rank, mpi_size, (time.time() - time0))
)
